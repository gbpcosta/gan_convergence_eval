...
def sigmoid(x, a=1., b=1., c=0.):
    return (a / (b+tf.exp(-x))) + c


def periodic_padding(image, padding=1):
    '''
    Create a periodic padding (wrap) around the image,
    to emulate periodic boundary conditions
    '''

    _, columns, _, _ = image.shape
    ...
    right_left_side = \
        tf.concat([right_filled_zeros, right_corner_diagonal], axis=0)
    right_center_diagonal = \
        tf.reshape(tf.eye(columns.value), [columns.value, columns.value])
    right_right_side = \
        tf.concat([right_corner_diagonal, right_filled_zeros], axis=0)

    right_matrix = \
        tf.concat([right_left_side, right_center_diagonal, right_right_side],
                  axis=1)

    # left and right matrices are immutable
    padded_image = tf.matmul(image, right_matrix)
...
def conv_cond_concat(x, y):
    ...
    return concat([x, y*tf.ones([x_shapes[0], x_shapes[1], x_shapes[2],
                                 y_shapes[3]])], 3)
